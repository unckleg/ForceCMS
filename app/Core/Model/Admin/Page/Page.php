...
namespace Core\Model\Admin\Page;

use ForceCMS\Model\Language as Language;
...
class Page extends \Zend_Db_Table_Abstract
{
    ...
    public function getAllWithContent() {
        ...
        $select->from('cms_page', ['id', 'status', 'deleted']);
    ...
    public function getAllWithContentAndNavigation() {
        ...
        $select = $this->select();
        $select->setIntegrityCheck(false);
        $select->from('cms_page', ['id', 'navigation_id', 'status', 'deleted']);
        $select->join('cms_page_content', 'cms_page_content.page_id = cms_page.id');
