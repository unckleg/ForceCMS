...
namespace ForceCMS\Collections\Markup;
...
class HtmlCompact
{
	...
	private static function _removeHtmlComments($html)
	{
		...
		if($keepCond) {
			$html = str_replace(['<!–[if', ''], ['–**@@IECOND-OPEN@@**–', '–**@@IECOND-CLOSE@@**–'], $html);
		}
		// Remove comments
		$html = preg_replace('//', '', $html);
		// Re sub-in the conditionals if required.
		if ($keepCond) {
			$html = str_replace(['–**@@IECOND-OPEN@@**–', '–**@@IECOND-CLOSE@@**–'], ['<!–[if', ''], $html);
		}
		return $html;
	}
