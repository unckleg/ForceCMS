...
class AclResource extends Mapper
{
    protected $Model_DbTable = 'ForceCMS\Controller\Plugin\Model\DbTable\Acl';
    ...
    public static function resourceExists($controller = null, $action = null) {
        ...
        $result = self::getBySQLCondition(['controller=?' => $controller,'action=?' => $action]);
        if(count($result)){
            return true;
        }
        return false;
