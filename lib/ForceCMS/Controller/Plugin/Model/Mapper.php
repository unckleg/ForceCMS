...
namespace ForceCMS\Controller\Plugin\Model;
...
class Mapper
{
    ...
    protected function MapperSave(array $data)
    {
        ...
        if (null === $id ) {
            ...
            $this->getDbTable(false)->update($data, ['id = ?' => $id]);
            $out = $id;
        }
        return $out;
    }
 
    protected function MapperGetByColumn($column = null, $value = null, $order = ['id DESC']){
        if(!$column || !$value)
    ...
    protected function MapperDeleteByColumn($column = null, $value = null){
        ...
        $result = $this->getDbTable()->delete(["$column=?" => $value]);
        return $result;
    }
     ...
     */
    protected function MapperGetBySQLCondition(array $conditions, $use_OR = false, $order = ['id DESC']){
        if(!count($conditions))
    ...
    protected function MapperFetchAll($order = ['id DESC'])
    {
