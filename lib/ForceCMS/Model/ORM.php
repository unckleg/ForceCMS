...
class ORM
{
    ...
    const TYPE_NUMERIC = ['int', 'tinyint', 'smallint', 'mediumint', 'bigint', 'decimal', 'float', 'double', 'real', 'bit', 'boolean', 'serial'];
    const TYPE_TEXT = ['char', 'varchar', 'tinytext', 'longtext', 'binary', 'varbinary', 'tinyblob', 'mediumblob', 'blob', 'longblob', 'enum', 'set'];
    const TYPE_DATE = ['date', 'datetime', 'timestamp', 'time', 'year'];
    ...
    public static function Mapper_Search(\Zend_Db_Table_Abstract $dbObject, array $parameters, $fetchType = null)
    {
        ...
        if (isset($parameters['limit'])) {
            if (isset($parameters['pagination'])) {
                // if param isset do limit by page
                $select->limitPage($parameters['pagination'], $parameters['limit']);
            } else {
                // param is not set do regular limit
     ...
     * @throws Exception
     */
    public static function Mapper_SearchBySqlQuery($sql, array $params = [])
    {
        if(!$sql) {
